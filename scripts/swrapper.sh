#!/bin/sh

# Authors:
# - Moritz Warning <moritzwarning@web.de> (2016)
# - Zhong Jianxin <azuwis@gmail.com> (2014)
#
# See file LICENSE at the project root directory for license information.
#
# i3status.conf should contain:
# general {
#   output_format = i3bar
# }
#
# i3 config looks like this:
# bar {
#   status_command exec /usr/share/doc/i3status/contrib/net-speed.sh
# }
#
# Single interface:
# ifaces="eth0"
#
# Multiple interfaces:
ifaces="wlp3s0 enp0s25 wwp0s20u4i6"
#

# Auto detect interfaces
#ifaces=$(ls /sys/class/net | grep -E '^(eth|wlan|enp|wlp)')

last_time=0
last_rx=0
last_tx=0
rate=""
title=""

readable() {
	local bytes=$1
	local kib=$(( bytes >> 10 ))
	if [ $kib -lt 0 ]; then
		echo "? K"
	elif [ $kib -gt 1024 ]; then
		local mib_int=$(( kib >> 10 ))
		local mib_dec=$(( kib % 1024 * 976 / 10000 ))
		if [ "$mib_dec" -lt 10 ]; then
			mib_dec="0${mib_dec}"
		fi
		echo "${mib_int}.${mib_dec} M"
	else
		echo "${kib} K"
	fi
}

update_stuff() {
	local time=$(date +%s)
	local rx=0 tx=0 tmp_rx tmp_tx

	for iface in $ifaces; do
		read tmp_rx < "/sys/class/net/${iface}/statistics/rx_bytes"
		read tmp_tx < "/sys/class/net/${iface}/statistics/tx_bytes"
		rx=$(( rx + tmp_rx ))
		tx=$(( tx + tmp_tx ))
	done

	local interval=$(( $time - $last_time ))
	if [ $interval -gt 0 ]; then
		rate="$(readable $(( (rx - last_rx) / interval )))↓ $(readable $(( (tx - last_tx) / interval )))↑"
	else
		rate=""
	fi

	last_time=$time
	last_rx=$rx
	last_tx=$tx

	if [ -e "/home/luhan/.config/i3/vlc.sock" ]; then
		echo -e "\n" | socat UNIX-CLIENT:/home/luhan/.config/i3/vlc.sock STDIO > /dev/null;
		title="$( echo -e get_title\\nget_length\\nget_time | socat UNIX-CLIENT:/home/luhan/.config/i3/vlc.sock STDIO | col -bx | tr '\n' ' ' | tr '\"' "'" )"

#		title="$( echo -e get_title | socat UNIX-CLIENT:/home/luhan/.config/i3/vlc.sock STDIO | col -bx | tr '\n' ' ' )"
#		title="$title$(echo -e get_time | socat UNIX-CLIENT:/home/luhan/.config/i3/vlc.sock STDIO | col -bx | tr '\n' ' ')/"
#		title="$title $(echo -e get_length | socat UNIX-CLIENT:/home/luhan/.config/i3/vlc.sock STDIO | col -bx | tr '\n' ' ')"

# YES, everything in that line needs to be exacly like that
# "col" is needed to remove some weird control characters
# tr is needed to remove newlines
# and to escape double-quotes
	else
		title=""
	fi
}

# for i3status "i3bar" output

# the reason why there are three lines here:
# the oldest version (on the bottom) skipped a round of the custom stuff, and displayed only the i3status output the first time it displays a line
# the next version only includes the network rate and is somewhat copied from the script from that one guy
# the currently active line includes the title for the currently playing vlc track

# i might try to rearrange the json blocks to group them according to displayed content (network, music/sound, other stuff)

i3status | (read line && echo "$line" && read line && echo "$line" && read line && update_stuff && echo "[{\"full_text\":\"${title}\" },{\"full_text\":\"${rate}\" },${line#\[}" && while :
#i3status | (read line && echo "$line" && read line && echo "$line" && read line && update_stuff && echo "[{\"full_text\":\"${rate}\" },${line#\[}" && while :
#i3status | (read line && echo "$line" && read line && echo "$line" && read line && echo "$line" && update_rate && while :
do
	read line
	update_stuff
	echo ",[{\"full_text\":\"${title}\" },{\"full_text\":\"${rate}\" },${line#,\[}" || exit 1
#	echo ",[{\"full_text\":\"${rate}\" },${line#,\[}" || exit 1
done)

# for i3status "none" output

#i3status | (while :
#do
#	read line
#	update_stuff
#	echo -en "$title"
#	echo -en " | "
#	echo -en "$rate"
#	echo -en " | "
#	echo -e "$line" || exit 1
##	 " $title | $rate | $line" || exit 1
#done)
